...
namespace VDETools
{
    public class ExportPdf
    {
        ...
        [DeclareAction("TestVersie")]
        private bool FilterExists(string filter)
        {
            Settings settings = new Settings();
            var settingsUrlScripts = "USER.PDFExportGUI.SCHEMAS";
            int countOfScripts = settings.GetCountOfValues(settingsUrlScripts);
            for (int i = 0; i < countOfScripts; i++)
            {
                MessageBox.Show(settings.GetStringSetting(settingsUrlScripts, i);)
            }
        }
    }
}